...
export function buildCopilotNarrative(vuln: VulnerabilityModel | null, report: ReportModel | null): string {
  ...
  return [
    ...
    `确定性证据：补丁验证状态=${valueText(verification.status)}，patch-related methods=${valueText(verification.patch_related_method_count)}，returncode=${valueText(verification.returncode)}。`,
    `分析范围：${valueText(targetScope.class_count, "0")} 个候选类，来源=${valueText(targetScope.source, "库版本识别候选类")}。`,
