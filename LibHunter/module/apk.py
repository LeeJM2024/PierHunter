...
descriptor_index_version = 2
empty_descriptor_token = "__EMPTY_METHOD_SIGNATURES__"
_DESCRIPTOR_TOKEN_PATTERN = re.compile(r"\{([^{}]+)\}")


def _descriptor_tokens_from_signature(signature) -> set:
    tokens = set()
    for raw in _DESCRIPTOR_TOKEN_PATTERN.findall(str(signature or "")):
        value = raw.strip()
        if not value:
            continue
...
class Apk(object):
    ...
    def __init__(self, apk_path, logger):
        ...
        self.descriptor_index = {"interface": {}, "concrete": {}, "all": {}}
        self.class_descriptor_tokens = {}
        self.descriptor_index_version = descriptor_index_version
    ...
    def build_descriptor_index(self):
        descriptor_index = {"interface": {}, "concrete": {}, "all": {}}
        class_descriptor_tokens = {}

        def _add(index_name, token, class_name):
            descriptor_index[index_name].setdefault(token, set()).add(class_name)

        for class_name, class_info in self.classes_dict.items():
            if len(class_info) == 2:
                class_kind = "interface"
                method_sigs = class_info[0]
            else:
                class_kind = "concrete"
                method_sigs = class_info[4]

            tokens = set()
            for sig in method_sigs:
                tokens.update(_descriptor_tokens_from_signature(sig))

            class_descriptor_tokens[class_name] = tokens
            if not tokens:
                _add(class_kind, empty_descriptor_token, class_name)
                _add("all", empty_descriptor_token, class_name)
            for token in tokens:
                _add(class_kind, token, class_name)
                _add("all", token, class_name)

        self.descriptor_index = descriptor_index
        self.class_descriptor_tokens = class_descriptor_tokens
        self.descriptor_index_version = descriptor_index_version
