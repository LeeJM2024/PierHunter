...
def _guava_bucket(version: str) -> str:
    match = re.match(r"^(\d+)(?:\.|$)", version)
    if not match:
        raise ValueError(f"Cannot derive major bucket from Guava version: {version}")
    bucket = f"{match.group(1)}.x"
    if version.endswith("-android"):
        return bucket + "-android"
...
def build_guava_bucket_pickles(
    pickle_dir: str,
    tpl_dex_dir: str,
    output_dir: str,
    threshold: float = 0.8,
...
def parse_args(argv: Optional[Sequence[str]] = None):
    parser = argparse.ArgumentParser(description="Build Guava major bucket skeleton pickles")
