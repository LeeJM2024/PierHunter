...
from engine.detector import prewarm_phunter_templates_from_cve_kb
...
def main() -> int:
    ...
    mode_group.add_argument("--apk", help="Target APK file path")
    mode_group.add_argument(
        "--prewarm-phunter",
        action="store_true",
        help="Only prewarm PHunter template caches from cve_kb.json",
    )
    parser.add_argument(
        "--prewarm-limit",
        type=int,
        default=0,
        help="Optional cap for prewarm count (0 means all records)",
    )
    args = parser.parse_args()

    try:
        if args.prewarm_phunter:
            ensure_runtime_dirs()
            print_banner()
            limit = args.prewarm_limit if args.prewarm_limit > 0 else None
            started = time.time()
            summary = prewarm_phunter_templates_from_cve_kb(limit=limit)
            elapsed = time.time() - started
            print("[*] PHunter 模板预热完成")
            print(json.dumps(summary, ensure_ascii=False, indent=2))
            print(f"[*] Elapsed: {elapsed:.2f}s")
            return 0 if summary.get("failed", 0) == 0 and summary.get("hung", 0) == 0 else 1
