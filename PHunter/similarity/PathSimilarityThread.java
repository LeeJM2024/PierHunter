...
public class PathSimilarityThread implements Runnable {
    ...
    public PathSimilarityThread(float[] array, List<Node<PredicateNodeData>> tplPredicateList,
                                ...
                                MethodDigest app, String tplTailKind) {
        this.array = array;
        this.tplPredicateList = tplPredicateList;
        this.tplSignature = tplSignature;
        this.tplVariable = tplVariable;
        this.app = app;
        this.tplTailKind = tplTailKind;
    }

    @Override
    public void run() {
        int appPathCount = Math.min(
                app.getPathCount(),
                Math.min(
                        app.realPredicates.size(),
                        Math.min(app.realSignatures.size(), app.realVariables.size())
                )
        );
        for (int j = 0; j < appPathCount; j++) {
            String appTailKind = app.getPathTailKind(j);
            if (!MethodDigest.isTailKindCompatible(tplTailKind, appTailKind)) {
                continue;
            }
