...
public class MainClass {
    ...
    private static final String OPTION_LIMIT_CLASSES = "limitClasses";
...
//                " the path should be a directory, each of apk will have a res file in the dir");
        ...
        options.addOption(OPTION_LIMIT_CLASSES, true, "Comma-separated classes/packages to limit APK analysis scope");
        ...
        options.addOption(OPTION_PREWARM_ONLY, false, "Build template caches only and exit");
        options.addOption(OPTION_PREWARM_APK_ONLY, false, "Build APK analysis cache only and exit");
    }
...
//            // 2 analyze the patch to extract patch-related method for location
            PatchSummary patchSummary = PatchSummary.loadOrCreate(config, pre, post, logger);

            if (cmd.hasOption(OPTION_PREWARM_ONLY)) {
                logger.info("PHunter prewarm mode completed. Template caches are ready.");
                return;
            }
...
//             3 analyze the target obfuscated app(maybe more than one)
    ...
    protected void parseCommandOptions(CommandLine cmd, Configuration config) throws IOException {
        ...
        String limitClasses = cmd.getOptionValue(OPTION_LIMIT_CLASSES);
        if (limitClasses != null && !limitClasses.isEmpty())
            config.setTargetClasses(limitClasses);
